...
page_init(void)
{
	...
	// free pages!
	size_t i;
    physaddr_t kern_memory_end = PADDR(boot_alloc(0));
	for (i = 0; i < npages; i++) {
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
    ...
    uint32_t ending = (uint32_t) ROUNDUP(va + len, PGSIZE);
    uint32_t i;
    for (i = begin; i <ending; i+=PGSIZE){
        pte_t *pte = pgdir_walk(env->env_pgdir, (void*)i, 0);
		//struct PageInfo *pt = page_lookup(env->env_pgdir, i, &pte);
        //pprint(pte);
